...
import { getCartData } from "../../services/cartService"; // your axios functions
import "./Cart.css";

const Cart = () => {
  const navigate = useNavigate();
  const { foodList, increaseQty, decreaseQty, quantities, removeFromCart, setQuantities } = useContext(StoreContext);
  ...
  useEffect(() => {
    const fetchCart = async () => {
      try {
        ...
        if (cartData) {
          // Update your context quantities
                  ...
                  <div key={food.id} className="row cart-item mb-3">
                    ...
                    <div className="col-md-2">
                      <div className="input-group">
                        <button className="btn btn-outline-secondary btn-sm" onClick={() => decreaseQty(food.id)}>-</button>
                        <input
                          ...
                          readOnly
                        />
                        <button className="btn btn-outline-secondary btn-sm" onClick={() => increaseQty(food.id)}>+</button>
                      </div>
                    </div>
